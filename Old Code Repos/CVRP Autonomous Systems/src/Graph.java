...
public class Graph extends JPanel {
  ...
  public static final Color POINT_COLOUR = Color.GREEN;
  ...
  private void setWidthAndHeight() {
    ...
    for (int i = 1; i < CVRPData.NUM_NODES; i++) {
      xPos = (int) CVRPData.getLocation(i-1).getX();
      yPos = (int) CVRPData.getLocation(i-1).getY();
  ...
  protected void paintComponent(Graphics g) {
	...
	int x1 = ((int) CVRPData.getLocation(trucks.get(i).get(j).intValue()-1).getX()     * SCALE_FACTOR) + PADDING;
	int y1 = HEIGHT - ((int) CVRPData.getLocation(trucks.get(i).get(j).intValue()-1).getY()     * SCALE_FACTOR) + PADDING;
	int x2 = ((int) CVRPData.getLocation(trucks.get(i).get(j + 1).intValue()-1).getX() * SCALE_FACTOR) + PADDING;
	int y2 = HEIGHT - ((int) CVRPData.getLocation(trucks.get(i).get(j + 1).intValue()-1).getY() * SCALE_FACTOR) + PADDING;
    ...
    // Draw depot
    drawX(g2d, DEPOT_COLOUR, CVRPData.getLocation(0), 1);

    // Draw customers
    for (int i = 2; i <= CVRPData.NUM_NODES; i++) {
      drawX(g2d, POINT_COLOUR, CVRPData.getLocation(i-1), i);
    }
  }
  ...
  protected void drawX(Graphics2D g, Color c, Point p, int label) {
    int x = (((int) p.getX()) * SCALE_FACTOR) + PADDING;
    int y = PADDING + HEIGHT - (((int) p.getY()) * SCALE_FACTOR);
    String str = ""+label;
    g.setColor(c);
    g.drawString(str, x+ 2*CROSS_SIZE,y+ 2*CROSS_SIZE);
